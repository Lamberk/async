...
from curses_tools import draw_frame, get_random_position, read_controls
from explosion import explode
from game_scenario import PHRASES, get_garbage_delay_tics
from obstacles import Obstacle, add_obstacle, has_collision
from physics import update_speed
from utils import (
    draw_plasma_gun_frame,
    get_center_position,
    get_new_coordinates,
    get_star_symbol,
)
...
async def animate_spaceship(canvas, start_row, start_column):
    ...
    for rocket_frame in itertools.cycle(
        ...
        row, column = get_new_coordinates(state, canvas, row, column, rocket_frame)
...
async def fly_garbage(canvas, column, garbage_frame, speed=0.5):
    ...
    column = max(column, 0)
    column = min(column, columns_number - columns_size - GARBAGE_RIGHT_SIDE_OFFSET)
    row = 1

    obstacle = add_obstacle(state, row, column, rows_size, columns_size)

    while row < rows_number - rows_size - 1:
...
async def draw_year(canvas):
    empty_phrase = " " * (
        len(max(PHRASES.values(), key=lambda e: len(e))) + len(str(max(PHRASES.keys())))
    )
    ...
    while not state.is_game_over:
        ...
        elif state.year > max(PHRASES.keys()):
            phrase = f": {PHRASES[max(PHRASES.keys())]}"
        else:
            phrase = empty_phrase
